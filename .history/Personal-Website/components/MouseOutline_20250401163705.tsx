  ...
  useEffect(() => {
    ...
    const handleMouseEnter = (e: MouseEvent) => {
      ...
      if (interactiveElement) {
        ...
        // Handle different border radius formats (px, rem, em)
        let radiusValue = 0
        if (borderRadius.includes('px')) {
          radiusValue = parseInt(borderRadius)
        } else if (borderRadius.includes('rem')) {
          const remValue = parseFloat(borderRadius)
          const rootFontSize = parseFloat(getComputedStyle(document.documentElement).fontSize)
          radiusValue = remValue * rootFontSize
        } else if (borderRadius.includes('em')) {
          const emValue = parseFloat(borderRadius)
          const parentFontSize = parseFloat(getComputedStyle(interactiveElement.parentElement!).fontSize)
          radiusValue = emValue * parentFontSize
        }

  ...
  useEffect(() => {
    ...
    const handleMouseEnter = (e: MouseEvent) => {
      ...
      if (interactiveElement) {
        ...
        // If the element is a link, try to get the border radius from its parent
        if (interactiveElement.tagName === 'A') {
          const parent = interactiveElement.parentElement
          if (parent) {
            const parentStyle = window.getComputedStyle(parent)
            const parentRadius = parentStyle.borderRadius
            if (parentRadius !== '0px') {
              setTargetRadius(parseInt(parentRadius))
            } else {
              setTargetRadius(parseInt(borderRadius))
            }
          } else {
            setTargetRadius(parseInt(borderRadius))
          }

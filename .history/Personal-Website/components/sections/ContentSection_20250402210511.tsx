...
export default function ContentSection() {
  ...
  // Track active section for the scroll indicator
  ...
  // Handle scroll to section when indicator is clicked
  const scrollToSection = (section: string) => {
    switch(section) {
      case 'intro':
        lenis?.scrollTo(introBannerRef.current as HTMLElement)
        break
      case 'about':
        lenis?.scrollTo(aboutMeRef.current as HTMLElement)
        break
      case 'whatido':
        lenis?.scrollTo(whatIDoRef.current as HTMLElement)
        break
      case 'capabilities':
        lenis?.scrollTo(capabilitiesRef.current as HTMLElement)
        break
    }
  }
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (introBannerRef.current && scrollPos < introBannerRef.current.getBoundingClientRect().bottom + window.scrollY) {
        setActiveSection('intro')
      } else if (aboutMeRef.current && scrollPos < aboutMeRef.current.getBoundingClientRect().bottom + window.scrollY) {
        setActiveSection('about')
      } else if (whatIDoRef.current && scrollPos < whatIDoRef.current.getBoundingClientRect().bottom + window.scrollY) {
  ...
  return (
    <section ref={sectionRef} className="relative pt-0 pb-48 bg-gray-950">
        ...
        {[
          ...
          <div key={section.id} className="relative group">
            <motion.button
              ...
              className={`w-3 h-16 rounded-full backdrop-blur-md transition-all duration-300 border ${
              ...
              whileTap={{ scale: 0.95 }}
            />
            <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-full pr-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
              <div className="bg-gray-900/80 backdrop-blur-sm py-1 px-3 rounded text-white text-sm whitespace-nowrap">
                {section.label}
              </div>
            </div>
          </div>
      ...
      <div className="container-custom relative">
        ...
        <div ref={collaborateRef} className="max-w-4xl mx-auto text-center pb-24">
          <motion.h2

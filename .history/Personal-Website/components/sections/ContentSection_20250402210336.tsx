...
export default function ContentSection() {
  ...
  // Add effect to track scroll and update active section
  useEffect(() => {
    if (!lenis || !introBannerRef.current || !aboutMeRef.current || !whatIDoRef.current || !capabilitiesRef.current) return
    ...
    const checkActiveSection = () => {
      ...
      const introRect = introBannerRef.current?.getBoundingClientRect()
      const aboutRect = aboutMeRef.current?.getBoundingClientRect()
      const whatIDoRect = whatIDoRef.current?.getBoundingClientRect()
      const capabilitiesRect = capabilitiesRef.current?.getBoundingClientRect()
      
      if (introRect && introRect.top <= halfViewport && introRect.bottom > halfViewport) {
        setActiveSection(0)
      } else if (aboutRect && aboutRect.top <= halfViewport && aboutRect.bottom > halfViewport) {
        setActiveSection(1)
      } else if (whatIDoRect && whatIDoRect.top <= halfViewport && whatIDoRect.bottom > halfViewport) {
        setActiveSection(2)
      } else if (capabilitiesRect && capabilitiesRect.top <= halfViewport && capabilitiesRect.bottom > halfViewport) {
        setActiveSection(3)
      }
    }
  ...
  const scrollToSection = (index: number) => {
    switch(index) {
      case 0:
        introBannerRef.current?.scrollIntoView({ behavior: 'smooth' })
        break
      case 1:
  ...
  return (
    <section ref={sectionRef} className="relative pt-0 pb-48 bg-gray-950">
      {/* Fixed Navigation Indicators - Styled as an always-visible overlay */}
      <div className="fixed right-8 top-1/2 -translate-y-1/2 z-[100] flex flex-col gap-4">
        {['Intro', 'About', 'What I Do', 'Capabilities'].map((label, index) => (
          <div 
            key={index}
            className="group relative flex items-center"
            onClick={() => scrollToSection(index)}
          >
            <div className={`w-3 h-16 rounded-full cursor-pointer transition-all duration-300 shadow-lg ${activeSection === index ? 'bg-white scale-110' : 'bg-white/30 hover:bg-white/70'}`} />
            <div className="absolute right-7 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gray-900/90 px-3 py-2 rounded-md text-white text-sm whitespace-nowrap shadow-xl">
              {label}
            </div>
          </div>
        ))}

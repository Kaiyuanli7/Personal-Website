...
export default function ContentSection() {
  ...
  const currentSection = useTransform(
    [introBannerProgress, aboutMeProgress, whatIDoProgress, capabilitiesProgress],
    ([intro, about, what, cap]: number[]) => {
      const progress = [intro, about, what, cap]
      const maxIndex = progress.reduce((maxIndex, currentValue, currentIndex, array) => 
        currentValue > array[maxIndex] ? currentIndex : maxIndex, 0)
      return maxIndex
    }
  )

  // Convert motion value to state for the page indicator

    ...
    <section ref={sectionRef} className="relative pt-0 pb-48 bg-gray-950">
      ...
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-4">
        {['Intro', 'About', 'What I Do', 'Capabilities'].map((label, index) => (
          ...
          >
            <div className={`w-3 h-20 rounded-full cursor-pointer transition-all duration-300 ${activeSection === index ? 'bg-white' : 'bg-white/30 hover:bg-white/50'}`} />
            <div className="absolute right-7 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gray-800 px-3 py-1 rounded text-white text-sm whitespace-nowrap">
              {label}
            </div>
          </div>
        ))}

            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors duration-300 cursor-hover ${
                    cursorEnabled ? 'bg-white shadow-[0_0_10px_rgba(255,255,255,0.3)]' : 'bg-white/20'
                  }`}
                  aria-checked={cursorEnabled}
                  role="switch"
                >
                  <span className="sr-only">Enable cursor outline</span>
                  <motion.span
                    layout
                    className={`${
                      cursorEnabled ? 'bg-gray-950' : 'bg-white'
                    } inline-block h-5 w-5 transform rounded-full transition-transform duration-200 ease-in-out`}
                    animate={{
                      x: cursorEnabled ? 20 : 4
                    }}
                  />
                </button>
              </div>

...
export default function Cursor() {
  const { isHovering, setIsHovering, cursorEnabled, cursorColor } = useCursor()
  ...
  // Don't render on touch devices or if cursor is disabled
  if (
  ...
  // Get the current cursor color class from the context
  const cursorColorClass = CURSOR_COLORS[cursorColor]
  ...
  return (
    <>
      ...
      >
        <div className={`w-full h-full border ${cursorColorClass}`} 
      ...
      >
        <div className={`w-full h-full border rounded-full ${cursorColorClass}`}></div>
      </motion.div>
